...
class PrintScheduler:
    ...
    async def _start_print(self, db: AsyncSession, item: PrintQueueItem):
        ...
        # Reuse a previous completed print when its SD-card copy was moved to
        # the inert `.cached` name by the finish handler. Restoring the exact
        # upload path both avoids the firmware phantom-print bug and skips a
        # potentially large FTP transfer.
        ...
        try:
            ...
            logger.debug("Queue item %s: Cache lookup failed for %s: %s", item.id, cached_remote_path, e)

        if cache_hit:
            logger.info("Queue item %s: FTP cache hit for %s; skipping upload", item.id, remote_path)
        else:
            # Delete an unexpected live-path file before upload (avoids 553).
            try:
        ...
        upload_operation = upload_file_async
        if cache_hit:
            async def _reuse_cached_file(*_args, **_kwargs) -> bool:
                return True

            upload_operation = _reuse_cached_file
        ...
        try:
            if ftp_retry_enabled:
                ...
                uploaded = await upload_operation(
                    printer.ip_address,
                    printer.access_code,
                    file_path,
                    remote_path,
